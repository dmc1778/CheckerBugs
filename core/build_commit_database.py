...
def main(lib_owner, lib_name, task):
    ...
    target_path = os.path.join('ml_repos', lib_name)
    
    if not os.path.exists(target_path):
        os.makedirs(target_path)

    try:
        subprocess.run(["git", "clone", MLREPOS_GITHUB_LINK[lib_name], target_path], check=True)
        print("Repository cloned successfully.")
    except subprocess.CalledProcessError as e:
        print(f"Error cloning repository: {e}")

    repo_path = f"ml_repos/{lib_owner.lower()}"
    ...
    if task == 'rag':
        ...
        data = pd.read_csv(f'data/taxonomy_data.csv', encoding='utf-8', sep=',')
    data_dict = {
        'train_data': data
        # 'test_data': test_df
    }
    ...
    if task == 'rag':
        ...
        if not os.path.exists(f'data/taxonomy_data'):
            os.makedirs('data/taxonomy_data')
        f = open(f'data/taxonomy_data/{lib_name}_test_data.json', 'a')
        f.write('[')
        
    for k, v in data_dict.items():
        for idx, row in v.iterrows():
            commit_hash = row.loc['Commit'].split('/')[-1]
